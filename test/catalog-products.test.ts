...
test('standard product aggregation keeps the highest-priority source and chooses its lowest offer', () => {
  const products = aggregateCatalogProducts([
    ...
    {
      standardProduct: 'chatgpt-plus', platform: 'ChatGPT', productType: 'subscription', categoryName: 'ChatGPT', name: 'ChatGPT Plus',
      price: '¥1', priceNumber: 1, priceUnit: '¥', currencyCode: 'CNY', inStock: true, refreshedAt: '2026-08-28T01:30:00.000Z',
      refreshTime: '', siteId: 'merchant-b', siteName: '商家 B', siteUrl: '', siteProductRefreshSuccessAt: null, siteProductRefreshSuccessTime: '', siteScore: 50,
      siteSponsor: false, clickCount: 0, score: 0, sourceName: '同站点低优先级', sourcePriority: 10, isSample: true,
    },
    {
      standardProduct: 'chatgpt-plus', platform: 'ChatGPT', productType: 'subscription', categoryName: 'ChatGPT', name: 'ChatGPT Plus',
  ...
  assert.equal(products[0].channelCount, 3, 'different sites remain independent channels even at a lower source priority');
  assert.equal(products[0].availableChannelCount, 2);
  assert.equal(products[0].channelDetails?.map(channel => channel.siteId).join(','), 'merchant-c,merchant-b,merchant-a');
